import openai as o, os, platform, random, re, sys, time, subprocess
...
def randomize_filename(os_type):
    timestamp = int(time.time())
    random_num = random.randint(10000, 99999)
    return f"darwin-{os_type}-{random_num}-{timestamp}.py"

def replicate(file_path, modified_code, os_type):
    new_file = randomize_filename(os_type)
...
def generator(n):
    for _ in range(n):
        ...
        os_type = get_os()
        replicate(file_path, modified_code, os_type)
